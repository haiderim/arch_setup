...
--country India,UnitedStates,Germany,Japan
--protocol https
--latest 20
--sort rate
--age 12
...
# Validation
checks=0
[[ -f "/boot/EFI/systemd/systemd-bootx64.efi" ]] && ((checks++))
[[ -f "/boot/vmlinuz-linux" ]] && ((checks++))
id "$USERNAME" &>/dev/null && ((checks++))
groups "$USERNAME" | grep -q wheel && ((checks++))
systemctl is-enabled systemd-networkd &>/dev/null && ((checks++))
echo "[INFO] Validation score: $checks/5"
[[ $checks -ge 4 ]] && echo "[SUCCESS] Pre-installation completed!" || echo "[WARNING] Some components may need manual verification"
CHROOT_EOF

log "Pre-install complete. Now switch to chroot and run post-install if needed."

ihaider@thin-srv-homelab-01:~/arch_setup$
ihaider@thin-srv-homelab-01:~/arch_setup$ cat pre_install.sh
#!/usr/bin/env bash
set -euo pipefail
